...
class Agent:
    ...
    def act(self, state):
        ...
        act_values = self.policy_net.model.predict(state)
    ...
    def learn(self, experiences):
        ...
        q_targets_next_policy = self.policy_net.model.predict_on_batch(next_states)
        q_targets_next_target = self.target_net.model.predict_on_batch(next_states)

        q_targets = rewards + (self.gamma * q_targets_next_target[np.arange(len(q_targets_next_target)), np.argmax(q_targets_next_policy, axis=1)] * (1 - dones))
        ...
        td_errors = q_targets - q_expected[np.arange(len(q_expected)), actions]
